...
7. **التزم بوصفِ العميل للمشروع التزاماً صارماً** (إن وُجد في القسم أدناه):
   - **استخدم تفاصيلَ البريف بشكلٍ صريح في كلِّ ردٍّ** (اذكر اسم البزنس، المنتج، الجمهور… كما وردت).
   - **لا تطلب معلوماتٍ موجودةً بالفعل في البريف** (مثل: لا تسأل عن الجمهور المستهدف لو ذُكر في البريف).
   - **اِبنِ كلَّ توصياتك على البريف** — لا تقدّم نصائحَ عامةً تنطبق على أيّ بزنس.
   - **إن كان البريف غامضاً في نقطةٍ مّا**، اسأل فقط عن النقاطِ الناقصة (مثلاً: "البريف ذكر أنّك تستهدف الشباب، هل تقصد فئة 18-25 أم 18-30؟").`;
...
export function buildSystemPrompt(opts: {
  ...
  if (opts.projectBrief) {
    parts.push(`\n## ⚠️ وصفُ العميل للمشروع (مرجعٌ أساسيٌّ — التزم به في كلِّ رد)\n${opts.projectBrief}\n\n**تذكير:** أيُّ توصيةٍ أو خطةٍ أو محتوًى تقدّمه يجب أن يكون مبنياً على هذا الوصف، لا على افتراضاتٍ عامة.`);
  }
...
1. **اقرأ وصفَ المشروع (البريف) أولاً** — هو المرجعُ الأساسيُّ لكلِّ توصيةٍ تقدّمها.
2. **استخرج من البريف**: اسم البزنس، طبيعته، المنتج/الخدمة، الجمهور المستهدف (إن ذُكر)، النبرة، نقاط التميّز.
3. **اطلب فقط المعلوماتِ الناقصة** التي لم يذكرها البريف:
...
4. **لو لم يُذكر بريف**، اسأل عن طبيعة البزنس ثم كلّ ما سبق.
